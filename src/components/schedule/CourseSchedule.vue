  ...
  <div class="info">
    ...
    <div class="item">
      <div class="h1" :class="{'active': isShow}" @click="showLessons">
        ...
        <span>{{courseInfo.circle && courseInfo.circle.circles ? courseInfo.circle.circles.title : ''}}</span>
        <div class="label label-one" v-if="courseInfo.type===2">补</div>
        <div class="label label-two" v-if="courseInfo.is_test===1">循环测</div>
      </div>
      <div class="Lessons" v-if="courseInfo.teacher && isShow">
        <p class="time">
          {{courseInfo.start_at | formatDate('YYYY.MM.DD')}}  {{courseInfo.start_at | formatDate('HH:mm')}}—{{courseInfo.end_at | formatDate('HH:mm')}}
        </p>
        <div class="courseTeacher">
          <router-link
            :to="{name: 'teacher',
            params: {'id': courseInfo.teacher.id}}"
            class="avatar">
            <img v-if="courseInfo.teacher.avatar_url !== ''" :src="courseInfo.teacher.avatar_url"/>
            <img v-else src="../../assets/default-photo-teacher.png"/>
          </router-link>
          <span class="name">{{courseInfo.teacher.realname}}</span>
        </div>
      </div>
      <div
        class="flow"
        v-if="contentType !== 3 || (contentType === 3 && !courseInfo.circle_progress.testing.show)"
        v-show="isShow">
        <div
          :class="{'flow-done': isFlowOne, 'flow-wait': !isFlowOne}"
          v-show="courseInfo.circle_progress.lecture.show > 0">
        </div>
        <p
        ...
        <div
          ...
          v-show="courseInfo.circle_progress.preview.show > 0">
        </div>
        <p
        ...
        <div
          ...
          v-show="courseInfo.circle_progress.learn_status.show > 0">
        </div>
        <p
        ...
        <div
          ...
          v-show="courseInfo.circle_progress.homework.show">
        </div>
        <p
      ...
      <div class="btn-fn" v-show="isShow">
        <router-link
          ...
          v-if="courseInfo.course.type !== 6 && (contentType === 2 || contentType === 4 || contentType === 5 || contentType === 8 || contentType === 9)"
        ...
        <span
          v-if="contentType !== 3"
          class="fn-item"
          :class="{'highlight': courseInfo.circle_progress.target.finish || contentType === 2 || contentType === 4 || contentType === 5 || contentType === 8 || contentType === 9}"
          @click="goAims()">
          课堂目标
        </span>
        <span
          v-if="(contentType === 3 && !courseInfo.circle_progress.testing.show) || contentType === 1 || contentType === 6 || contentType === 7"
          class="fn-item"
          :class="{'highlight': courseInfo.circle_progress.teacher_comment.show &&
          courseInfo.circle_progress.teacher_comment.finish}"
          @click="goClassReport()">
          课堂报告
        </span>
        <span
      ...
      <div
          v-show="isShow"
          v-if="contentType === 3 || contentType === 1 || contentType === 6 || contentType === 7"
          class="fn-comment"
          @click="evaluate()">
          <div :class="{'highlight': (courseInfo.circle_progress.comment_teacher.show || courseInfo.circle_progress.comment_teacher.finish), 'new-comment': (!courseInfo.circle_progress.comment_teacher.show && !courseInfo.circle_progress.comment_teacher.finish)}"></div>
          评价老师
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    data() {
      return {
        isShow: false,
        // flow
        isFlowOne: this.courseInfo.circle_progress.lecture.finish > 0,
        isFlowTwo: this.courseInfo.circle_progress.preview.finish > 0,
        isFlowThree: this.courseInfo.circle_progress.learn_status.finish > 0,
        isFlowFour: this.courseInfo.circle_progress.homework.finish > 0,
    ...
    methods: {
      ...
      // flow线长度
      flowLine() {
        if (this.courseInfo.circle_progress.lecture.show > 0) {
          this.arrList.push('');
        }
        if (this.courseInfo.circle_progress.preview.show > 0) {
          this.arrList.push('');
        }
        if (this.courseInfo.circle_progress.learn_status.show > 0) {
          this.arrList.push('');
        }
        if (this.courseInfo.circle_progress.homework.show > 0) {
          this.arrList.push('');
        }
        this.lineWidth = this.arrList.length === 1 ? 0 :
        (this.arrList.length === 2 ? 28 :
        ((this.arrList.length === 2 && this.courseInfo.circle_progress.homework.show > 0) ? 56.5 :

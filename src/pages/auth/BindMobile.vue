...
import { sendSmsCode, mobileBind, classroomCnt } from '@/api';
...
export default{
  ...
  methods: {
    ...
    sendCode() {
      ...
      this.tips = '';
      this.btnTxt = '发送中...';
      sendSmsCode(this.mobile, 2).then((res) => {
        if (res.error_code !== 0) {
          this.isSend = 0;
    ...
    confirm() {
      ...
      this.isSubmit = 1;
      this.subTxt = '绑定中...';
      classroomCnt(this.uid, this.mobile).then((res) => {
        if (res.data.cnt !== 0) {
          const mobile = `${this.mobile.substr(0, 3)}****${this.mobile.substr(7)}`;
          this.dialogData.tips = `当前账号${mobile}已有课程，绑定新手机号后，旧课程将不被同步到新账号上，是否确定继续？`;

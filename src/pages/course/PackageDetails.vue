<template>
  <div class="course-details">
    ...
    <div class="item-box">
      ...
      <ul class="servers-box" @click="showPop = true">
    ...
    <!-- 组合包：包含内容 -->
    <div class="item-box" v-if="type === 'package' && info.course.length > 0">
      <div class="comm-tit">包含内容</div>
      <ul class="content-box content-list">
        <li v-for="item in info.course"
            :key="item.id"
            @click="goDetails(item)">
          <p class="title">{{ item.title }}</p>
          <p class="intro">
            <span><i class="icon time"></i>{{item.start_at}} - {{item.end_at}}</span>
            <span><i class="icon lessons"></i>共{{ item.lessons_sum || item.lessons }}节</span>
            <span class="price">
              <em class="current">¥{{item.price | formatMoney}}</em>
            </span>
          </p>
        </li>

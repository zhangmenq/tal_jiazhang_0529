...
<div>
  ...
  <div class="course-details" v-else>
    ...
    <div class="nav-pay">
      ...
      <div>
        <x-button class="btn-signup" :class="{'btn-disable': !canBuy, 'btn-purple': canBuy}" @click.native="bookCourse()">
          {{ classroomId > 0 ? '已购买' : (orderId > 0 ? '未支付' : '立即报名') }}
        </x-button>
      </div>
    </div>
    <vx-dialog :show="dialogData.show" :tips="dialogData.tips" @onConfirm="goTest()" @onCancel="cancelTest()" class="test-dialog">
    </vx-dialog>
  ...
  .course-details {
    ...
    .item-box {
      ...
      .details-box {
        ...
        border-bottom: 0.01rem solid @col-border;
        p {
          line-height: 0.44rem;
          &:before {
            margin-right: 0.2rem;

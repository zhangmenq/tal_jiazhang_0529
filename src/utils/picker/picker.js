...
(function (window, document, Math) {
  ...
  Scroll.prototype = {
    _start: function () {
        let self = this;
        self.scroller.addEventListener('touchstart', (e) => {
            e.stopPropagation();
            e.preventDefault();
    ...
    _move: function () {
        let self = this;
        self.scroller.addEventListener('touchmove', (e) => {
            e.stopPropagation();
            e.preventDefault();
    ...
    _end: function () {
        let self = this;
        self.scroller.addEventListener('touchend', (e) => {
            e.stopPropagation();
            e.preventDefault();
            ...
            if (self.options.step && self.stepLen > 0) {
                ...
                let index = parseInt(Math.abs(moveY) / self.stepLen);
  ...
  Picker.prototype = {
    ...
    FillData: function () {
        ...
        self.enterNodesBlur();
          ...
          //body中是否存在picker
        let modalbox = document.querySelector('.zx_mask');
        if (modalbox) {
            document.body.removeChild(modalbox);
          }

        let title = self.options.title ? self.options.title : '请选择';
        let modalParent = document.createElement('div'); //父节点
        modalParent.className = 'zx_mask';

        let picker_list = `${'<div class="zx_select showPicker">' +
        ...
        document.body.appendChild(modalParent);
          ...
          //设置宽度
        let listWidth = `${parseFloat(100 / self.arrayDepth).toFixed(3)  }%`;
        let pickerWrapperArr = document.querySelectorAll('.picker-wrapper');
        for (let i = 0; i < pickerWrapperArr.length; i++) {
...
childData = [];//默认值
    ...
    eventClick: function () {
        let self = this;
          ...
          //取消按钮
        document.querySelector('.picker-cancel').addEventListener('touchstart', (e) => {
            e.stopPropagation();
            e.preventDefault();
          ...
          //确定按钮
        document.querySelector('.picker-ok').addEventListener('touchstart', (e) => {
            e.stopPropagation();
            e.preventDefault();
